...
Class Controller_Admin_Contact Extends Controller_Admin_Base 
{
	...
	function indexAction() 
	{
					...
					foreach($delArr as $id)
					{
						...
						if($myContact->id > 0)
						{
							...
							if($myContact->delete())
							{
								...
								$this->registry->me->writelog('contactdelete', $myContact->id, array('fullname' => $myContact->fullname, 'email' => $myContact->email, 'phone' => $myContact->phone, 'reason' => $myContact->reason, 'status' => $myContact->status, 'message' => $myContact->message));  	
							}
							else
		...
		if(strlen($keywordFilter) > 0)
		{
			...
			else if($searchKeywordIn == 'phone')
			{
				$paginateUrl .= 'searchin/phone/';
			}
			else if($searchKeywordIn == 'message')
			{
				$paginateUrl .= 'searchin/message/';
			}
			else if($searchKeywordIn == 'ipaddress')
			{
				$paginateUrl .= 'searchin/ipaddress/';
			}
			else if($searchKeywordIn == 'note')
			{
		...
		$this->registry->smarty->assign(array(	'menu'		=> 'contact',
	...
	function editAction()
	{
		...
		if($myContact->id > 0)
		{
			...
			$formData['fuserid'] = $myContact->userId;
			$formData['fusername'] = $myContact->username;
			...
			if(!empty($_POST['fsubmit']))
			{
				...
				if($this->editActionValidator($formData, $error))
				{
					...
					if($myContact->updateData($error))
					{
						...
						$this->registry->me->writelog('contactedit', $myContact->id);
