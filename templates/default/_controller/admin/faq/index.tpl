...
<div class="content-box"><!-- Start Content Box -->
	<div class="content-box-header">		
		...
		<ul class="content-box-link">
			<li><a href="{$conf.rooturl_admin}faq/add">{$lang.controller.head_add}</a></li>
		</ul>
		{if $formData.search != ''}
		<ul class="content-box-link">
			<li><a href="{$conf.rooturl_admin}faq">{$lang.controllergroup.formViewAll}</a></li>
		</ul>
		{/if}
		<div class="clear"></div>  
			...
			<form action="" method="post" name="manage" onsubmit="return confirm('Are You Sure ?');">
				...
				{if $entries|@count > 0}
					...
					{foreach item=entry from=$entries}
						...
						<tr>
							<td><input type="checkbox" name="fbulkid[]" value="{$entry->id}" {if in_array($entry->id, $formData.fbulkid)}checked="checked"{/if}/></td>
							<td style="font-weight:bold;">{$entry->id}</td>
							<td><input type="text" size="3" value="{$entry->order}" name="forder[{$entry->id}]" class="text-input" /></td>
							<td><a href="{$conf.rooturl_admin}faq/edit/id/{$entry->id}/redirect/{$redirectUrl}">{$entry->title.$langCode}</a></td>
							<td>{$entry->datecreated|date_format:$lang.controllergroup.dateFormatSmarty}</td>
							<td class="td_center">{if $entry->enable == 1}<img border="0" src="{$currentTemplate}/images/admin/icons/tick_circle.png" alt="{$lang.controllergroup.formYesLabel}" title="{$lang.controllergroup.formYesLabel}" width="16"/>{else}<img border="0" src="{$currentTemplate}/images/admin/icons/cross_circle.png" alt="{$lang.controllergroup.formNoLabel}" title="{$lang.controllergroup.formNoLabel}" width="16"/>{/if}</td>
							<td><a title="{$lang.controllergroup.formActionEditTooltip}" href="{$conf.rooturl_admin}faq/edit/id/{$entry->id}/redirect/{$redirectUrl}"><img border="0" src="{$currentTemplate}/images/admin/icons/pencil.png" alt="{$lang.controllergroup.formEditLabel}" width="16"/></a> &nbsp;
								<a title="{$lang.controllergroup.formActionDeleteTooltip}" href="javascript:delm('{$conf.rooturl_admin}faq/delete/id/{$entry->id}/redirect/{$redirectUrl}?token={$smarty.session.securityToken}');"><img border="0" src="{$currentTemplate}/images/admin/icons/cross.png" alt="{$lang.controllergroup.formDeleteLabel}" width="16"/></a>
							</td>
						</tr>
			...
			<form action="" method="post" style="padding:0px;margin:0px;" onsubmit="return false;">
				...
				{$lang.controller.formKeywordLabel}:
					...
					<input type="text" name="fkeyword" id="fkeyword" size="20" value="{$formData.fkeyword|@htmlspecialchars}" class="text-input" /><select name="fsearchin" id="fsearchin">
						...
						<option value="contents" {if $formData.fsearchin eq 'contents'}selected="selected"{/if}>{$lang.controller.formKeywordInContentsLabel}</option>
...
<script type="text/javascript">
	function gosearch()
	{
		var path = rooturl_admin + "faq/index";
