 ...
 */
define('CACHE_PATH_TO', SITE_PATH.'uploads' . DIRECTORY_SEPARATOR .'cache');
/**
...
class cache{
	...
	*/
	 ...
	 * @access protected
	 */
	function __construct($id = false, $cacheDir = '', $isCacheVariable = false) {
		if($id) {
			$this->set_id($id);
		}
		...
		if($cacheDir == '')
		{
			$this->path_to_directory = CACHE_PATH_TO;  
		}
		else
		{
	...
	private function get_path_to_file($id = false) {
		...
		$this->path_to_file = $this->path_to_directory.DIRECTORY_SEPARATOR.$this->id.'.'.CACHE_EXTENSION;
