	...
	function read($session_id)
	{
		...
		$sql = 'SELECT s_data 
				...
				WHERE s_id = ?
					...
					AND s_ip = ?
					';
		$session_data =  $this->db->query($sql, array($session_id, $_SERVER['HTTP_USER_AGENT'], Helper::getIpAddress()))->fetchSingle();
	...
	function write($session_id, $session_data)
	{
		...
		if($count > 0)
		{
			...
			$sql = 'UPDATE ' . TABLE_PREFIX . 'sessions
					...
					WHERE s_id = ?
					LIMIT 1';
			if($this->db->query($sql, array($session_data, time() + $this->sessionLifetime, $session_id))->rowCount() > 0)
			{
				return true;
			}
		}
		else 
		{
			...
			$sql = 'INSERT INTO ' . TABLE_PREFIX . 'sessions(`s_id`, `s_data`, `s_agent`, `s_ip`, `s_hash`, `s_dateexpired`, `s_datecreated`)
					VALUES(?, ?, ?, ?, ?, ?, ?)';
			if($this->db->query($sql, array($session_id, $session_data, $_SERVER['HTTP_USER_AGENT'], Helper::getIpAddress(), md5($_SERVER['HTTP_USER_AGENT'] . '-' . Helper::getIpAddress()), time() + $this->sessionLifetime, time()))->rowCount() > 0)
				return '';
		}
